...
grouped_sweeps = defaultdict(list)
for sweep in sweeps:
    grouped_sweeps[sweep["sweep_type"]].append(sweep)
...
for dataset in datasets:
    ...
    for sweep_type, sweep_group in grouped_sweeps.items():
        config_paths = []
        sweep_configs_info = []
        
        for sweep in sweep_group:
            alpha = sweep["alpha"]
            ...
            config_paths.append(temp_config_path)
            sweep_configs_info.append((config_name, sweep))
        ...
        tsv_path = f"results/{dataset}/results_{first_config_name}_{num_nodes}.tsv"
        
        if not os.path.exists(tsv_path):
            print(f"Error: Output file {tsv_path} not found!")
            continue
            
        # Read the entire TSV once
        with open(tsv_path, "r") as f:
            reader = list(csv.DictReader(f, delimiter="\t"))
            
        # Parse metrics for each config in the group
        for config_name, sweep in sweep_configs_info:
            alpha = sweep["alpha"]
            capacitance = sweep["capacitance"]
            ...
            if reader and asn_col in reader[0]:
                for row in reader:
                    val = row.get(asn_col)
