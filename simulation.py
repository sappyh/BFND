...
def setup_simulation_environment(config_params, run_seed_sequence, logger):
    ...
    for k, config_dict in enumerate(configs):
        ...
        for i in range(current_num_nodes):
            ...
            radio = RadioFactory.create_radio(radio_type="async", publisher=network_publishers[i], log_level=component_log_level)
...
async def main_async():
    ...
    # Compute results filepath early
    ...
    expected_cols = len(config_params_for_worker['config_names'])
    completed_runs = 0
    valid_rows = []
    ...
    if os.path.exists(results_filepath):
        try:
            ...
            if len(lines) > 0:
                ...
                expected_header = [f"ASN_{name}" for name in config_params_for_worker['config_names']]
                if header == expected_header:
                    ...
                    for line in lines[1:]:
                        ...
                        if len(parts) == expected_cols:
                            ...
                            # Stop reading if we find an incomplete/malformed row
                            break
        except Exception as e:
            main_logger.warning(f"Error reading existing results file: {e}. Starting fresh.")
            completed_runs = 0
            header_written = False
            valid_rows = []

    if completed_runs >= NUM_SIMULATIONS:
        main_logger.warning(f"Config '{config_base_name}' already has {completed_runs}/{NUM_SIMULATIONS} simulations completed. Skipping.")
        return

    # Write/Truncate the results file to contain only the valid rows
    try:
        if completed_runs > 0:
            main_logger.warning(f"Resuming config '{config_base_name}' from {completed_runs}/{NUM_SIMULATIONS} completed simulations.")
        ...
        main_logger.error(f"Error initializing results file: {e}")
        return
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        tasks = [
            ...
            for sim_num in range(completed_runs, NUM_SIMULATIONS)
        ]
        
        remaining_simulations = NUM_SIMULATIONS - completed_runs
        with tqdm(total=remaining_simulations, desc=f"Config {config_base_name}", position=0, leave=True, file=sys.stdout, mininterval=1.0, maxinterval=10.0, smoothing=0.1) as progress_bar:
    ...
    end_time = time.time()
    main_logger.warning(f"Completed remaining simulations for config '{config_base_name}' in {end_time - start_time:.2f} seconds.")
    main_logger.warning(f"Results saved to {results_filepath}")
